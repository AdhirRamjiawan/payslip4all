...
- **User**: Represents an employer account. Attributes: unique email address, hashed password, display name. Owns zero or more Companies. Owns the **CompanyOwner** role.
...
- **Employee**: Represents a person employed by a Company. Attributes: First Name, Last Name, ID Number (national identity document number), Employee Number (employer-assigned identifier), Start Date (employment commencement date), Occupation (job title / role), UIF Reference (Unemployment Insurance Fund registration reference), Monthly Gross Salary (decimal, required — one fixed salary figure used for every payslip generated for this employee), Company reference. An Employee has zero or more Payslips.
...
- Q: What line items appear on the payslip PDF? → A: Three named lines — Gross Earnings (from Employee.Monthly Gross Salary), UIF Deduction (calculated from gross), Net Pay (Gross Earnings − UIF Deduction).
