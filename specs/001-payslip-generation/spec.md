...
- **FR-020**: System MUST persist a payslip record containing: Gross Earnings, UIF Deduction, Total Loan Deductions, Total Deductions, Net Pay, Employee reference, pay period (month and year), generation timestamp, and one `PayslipLoanDeduction` snapshot row per active loan (capturing Description and Monthly Deduction Amount at generation time).
...
- **FR-022**: System MUST produce a downloadable PDF payslip document that includes: **Gross Earnings**, **UIF Deduction**, one line per active **Loan Deduction** (showing the loan description and monthly deduction amount), **Total Deductions**, **Net Pay**, payment date (last calendar day of the pay period month), employer company name, company address, employer UIF reference number, employer SARS PAYE number, employee full name, employee number, employee ID number, employee occupation, employee start date, employee UIF reference, and pay period. SA-compliance fields (UIF/SARS numbers, ID number, start date, UIF reference) render as `"—"` when not configured.
