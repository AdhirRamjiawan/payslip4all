...
public class FakeHostedPaymentProvider : IHostedPaymentProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var normalizedOutcome = (outcomeValue ?? "pending").Trim().ToLowerInvariant();
        ...
        switch (normalizedOutcome)
        {
            case "succeeded":
            case "success":
                if (!decimal.TryParse(amountValue, NumberStyles.Number, CultureInfo.InvariantCulture, out var confirmedAmount) ||
                    confirmedAmount <= 0m)
                {
            ...
            case "failed":
                result.Outcome = HostedPaymentOutcome.Failed;
                result.FailureCode = "failed";
                result.FailureMessage = "Payment failed.";
                result.DisplayMessage = "Payment failed. Your wallet was not credited.";
                return Task.FromResult(result);
